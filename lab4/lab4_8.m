...
histeq_IHFF = histeq(uint8(IHFF(img, threshold, a, b)));
histeq_BHFF = histeq(uint8(BHFF(img, threshold, n_BHFF, a, b)));
histeq_GHFF = histeq(uint8(GHFF(img, threshold, n_GHFF, a, b)));

% 先经过高频增强滤波，再进行直方图均衡化，显示结果图像
subplot(3,3,4);
imshow(histeq_IHFF, []);
title('理想高通滤波器-直方图均衡化图像');

subplot(3,3,5);
imshow(histeq_BHFF, []);
title('巴特沃斯高通滤波器-直方图均衡化图像');

subplot(3,3,6);
imshow(histeq_GHFF, []);
title('高斯高通滤波器-直方图均衡化图像');

% 先进行直方图均衡化，再经过高频增强滤波，显示结果图像
subplot(3,3,7);
imshow((IHFF(histeq(img), threshold, a, b)), []);
title('直方图均衡化-理想高通滤波器图像');

subplot(3,3,8);
imshow(BHFF(histeq(img), threshold, n_BHFF, a, b), []);
title('直方图均衡化-巴特沃斯高通滤波器图像');

subplot(3,3,9);
imshow(GHFF(histeq(img), threshold, n_GHFF, a, b), []);
